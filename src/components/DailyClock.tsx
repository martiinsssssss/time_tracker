...
export function DailyClock() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col gap-6">
      <h2 className="text-slate-800 font-bold text-lg">Jornada de hoy</h2>

      <div>
        <div className="flex items-baseline justify-between mb-1">
          <span className="text-3xl font-extrabold text-slate-800">{formatHM(dayMs)}</span>
          <span className="text-slate-400 text-sm">objetivo {settings.workdayHours}h</span>
        </div>
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1">
          <span className="text-xl font-bold text-slate-700">{formatHM(weekMs)}</span>
          <span className="text-slate-400 text-sm">semana · objetivo {settings.weeklyTargetHours}h</span>
        </div>
        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
          <div
